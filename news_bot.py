...
def obtine_stiri(interogare, categorie_nume):
    # REPARARE: Definim variabila la începutul funcției ca să fie accesibilă oriunde
    numar_articole = 0
    ...
    if "articles" in raspuns and len(raspuns["articles"]) > 0:
        for articol in raspuns["articles"]:
            if numar_articole >= 4:  # Ne oprim la top 4 articole
                break
            ...
            # Filtrăm articolele șterse sau incomplete
...
print(f"Fișierul {nume_fisier} a fost generat fără erori!")
