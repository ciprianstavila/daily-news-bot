        ...
        if "articles" in raspuns and len(raspuns["articles"]) > 0:
            for articol in raspuns["articles"]:
                ...
                if titlu and titlu != "[Removed]" and descriere and descriere != "[Removed]":
                    html_stiri += "<div class='news-card'>"
...
# Construim structura HTML bucățică cu bucățică folosind string-uri chioare, sigure
linii_html = [
    "<!DOCTYPE html>",
    "<html lang='ro'>",
    "<head>",
    ...
    "<style>",
    "body { font-family: 'Segoe UI', sans-serif; background-color: #f4f7f6; color: #333; margin: 0; padding: 20px; display: none; }",
    ...
    f"    var parolaCorecta = '{PAROLA_SECRETA}';",
    ...
    f"<div class='date'>Actualizat automat la data de: {datetime.now().strftime('%d %B %Y')}</div>",
    "</header>"
]

# Unim structura de bază
continut_html = "\n".join(linii_html) + "\n"

# Adăugăm blocurile dinamice de știri
...
# Închidem structura site-ului
continut_html += "\n</div>\n</body>\n</html>"

with open(nume_fisier, "w", encoding="utf-8") as f:
    f.write(continut_html)

print("index.html generat cu succes prin array izolator.")
